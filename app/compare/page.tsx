            ...
            <section className="rounded-[16px] border border-[#e5e2da] bg-white p-5">
              ...
              <div className="grid gap-4 lg:grid-cols-3">
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">{only1.map((slug) => { const badge = badges1.get(slug)!; return <Link key={slug} href={`/badges?badge=${slug}&tier=${encodeURIComponent(badge.tier)}`} className="inline-flex items-center gap-1.5 rounded-full border border-[#e5e2da] bg-[#faf9f5] py-1 pr-1.5 pl-1 text-[#1a1918] no-underline hover:border-[#1a1918]">{badge.imageUrl && <span className="relative h-4 w-4 shrink-0"><Image src={badge.imageUrl} alt="" fill sizes="16px" className="object-contain" /></span>}<span className="text-[8px] font-semibold">{badge.name}</span><span className={cn("rounded-full px-1.5 py-0.5 font-plex text-[5.5px] text-white", TIER_CLASS[badge.tier])}>{badge.tier === "Hall of Fame" ? "HOF" : badge.tier.toUpperCase()}</span></Link>; })}</div>
                </div>
                <div className="border-x-0 border-[#e5e2da] lg:border-x lg:px-4">
                  <p className="mb-2 border-b border-[#e5e2da] pb-2 font-plex text-[8px]">SHARED · {shared.length}</p>
                  <div className="space-y-1.5">{shared.map((slug) => { const a = badges1.get(slug)!; const b = badges2.get(slug)!; return <Link key={slug} href={`/badges?badge=${slug}`} className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 rounded-[7px] px-1 py-0.5 text-[#1a1918] no-underline hover:bg-[#faf9f5]"><span className={cn("rounded px-1.5 py-0.5 font-plex text-[6px] text-white", TIER_CLASS[a.tier])}>{a.tier === "Hall of Fame" ? "HOF" : a.tier.toUpperCase()}</span><span className="flex min-w-0 items-center justify-center gap-1.5">{a.imageUrl && <span className="relative h-4 w-4 shrink-0"><Image src={a.imageUrl} alt="" fill sizes="16px" className="object-contain" /></span>}<span className="truncate text-center text-[9px] font-semibold">{a.name}</span></span><span className={cn("rounded px-1.5 py-0.5 font-plex text-[6px] text-white", TIER_CLASS[b.tier])}>{b.tier === "Hall of Fame" ? "HOF" : b.tier.toUpperCase()}</span></Link>; })}</div>
                </div>
                <div>
                  <p className="mb-2 border-b border-[#e5e2da] pb-2 font-plex text-[8px] text-right">{p2.name.toUpperCase()} ONLY · {only2.length}</p>
                  <div className="flex flex-wrap justify-end gap-1.5">{only2.map((slug) => { const badge = badges2.get(slug)!; return <Link key={slug} href={`/badges?badge=${slug}&tier=${encodeURIComponent(badge.tier)}`} className="inline-flex items-center gap-1.5 rounded-full border border-[#e5e2da] bg-[#faf9f5] py-1 pr-1.5 pl-1 text-[#1a1918] no-underline hover:border-[#1a1918]">{badge.imageUrl && <span className="relative h-4 w-4 shrink-0"><Image src={badge.imageUrl} alt="" fill sizes="16px" className="object-contain" /></span>}<span className="text-[8px] font-semibold">{badge.name}</span><span className={cn("rounded-full px-1.5 py-0.5 font-plex text-[5.5px] text-white", TIER_CLASS[badge.tier])}>{badge.tier === "Hall of Fame" ? "HOF" : badge.tier.toUpperCase()}</span></Link>; })}</div>
                </div>
              </div>
            </section>
